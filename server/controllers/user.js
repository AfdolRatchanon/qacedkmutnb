    ...
    if (sta_id === 4 || sta_id == null) {
      ...
      if (type_id === 0 || type_id == null) {
        ...
        if (qst_title === "" || qst_title == null) {
          return res
            .status(400)
            .send("กรุณาแก้ไขข้อมูลก่อนกดยืนยัน หรือ กรุณากรอกหัวข้อคำถาม");
        } else {
          if (img_Dname === "" || qst_detail == null) {
            return res
              .status(400)
              .send("กรุณาแก้ไขข้อมูลก่อนกดยืนยัน หรือ กรุณากรอกรายระเอียด");
          } else {
            if (qst_detail === "" || qst_detail == null) {
              return res
                .status(400)
              ...
              if (qst_name === "" || qst_name == null) {
                ...
                if (qst_mail === "" || qst_mail == null) {
                  return res
                    .status(400)
                    .send("กรุณาแก้ไขข้อมูลก่อนกดยืนยัน หรือ กรุณากรอกอีเมล");
                } else {
        ...
        if (err) {
          ...
          if (result[0] == null) {
            ...
            if (result[0].sta_id != 3) {
              return res
                .status(400)
                .send("ไม่สามารถลบข้อมูลได้เนื่องจากมีการตอบคำถามกลับแล้ว");
            } else {
      ...
      async (err, result) => {
        if (err) {
          ...
          if (result.length > 0) {
            ...
            return res
              .status(400)
              .send("ชื่อผู้ใช้ หรือ อีเมล หรือ เบอร์โทรศัพท์ มีผู้ใช้งานแล้ว");
          }
